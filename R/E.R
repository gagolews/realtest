...
#' \code{value_comparer(object[["value"]], prototype[["value"]])} and
#' \code{sides_comparer(object[["sides"]], prototype[["sides"]])}
...
#'     \code{realtest_result}, e.g., \code{\link{stop_if_results_different}}
#'
#' @return
...
E <- function(
    ...
    for (i in seq_along(prototypes)) {
        ...
        cmp_value <- vcmp(object[["value"]], prototypes[[i]][["value"]])
        cmp_sides <- scmp(object[["sides"]], prototypes[[i]][["sides"]])
    ...
    names(matches) <- names(prototypes)[matches]  # works also when names is NULL
    ...
    postprocessor(ret)  # can, e.g., call stopifnot if matches are empty
}
